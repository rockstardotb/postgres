...
with beam.Pipeline(options=PipelineOptions()) as p:
        ...
        lines = p | 'ReadFile' >> beam.io.ReadFromText('gs://rockstardotb/zillow/Zip_MedianRentalPrice_5BedroomOrMore.csv')
...
with beam.Pipeline(options=PipelineOptions()) as p:
        ...
        lines = p | 'ReadFile1' >> beam.io.ReadFromText('gs://rockstardotb/zillow/Zip_MedianRentalPrice_5BedroomOrMore.csv')
...
with beam.Pipeline(options=PipelineOptions()) as p:
        ...
        lines = p | 'ReadFile3' >> beam.io.ReadFromText('gs://rockstardotb/zillow/Zip_MedianRentalPrice_5BedroomOrMore.csv')

        list_records = lines | 'CreateListRecords3' >> (beam.Map(parse_line2))

        list_records | 'WriteFile3' >> beam.io.WriteToText('/home/rockstardotb/tmp/list_records', file_name_suffix='.txt')
...
def run(argv=None):
      ...
      '--job_name=zillow-rentals-5bedroom' # assign descriptive name to this job, all in lower case letters
        ])
        ...
        with beam.Pipeline(options=pipeline_options) as p:
                ...
                table_name = "wide-fx-193021:zillow.Rental_Price_5BedroomOrMore" # format: project_id:dataset.table
                table_schema = init_bigquery_table()

                lines = p | 'ReadFile4' >> beam.io.ReadFromText('gs://rockstardotb/zillow/Zip_MedianRentalPrice_5BedroomOrMore.csv')
                list_records = lines | 'CreateListRecords4' >> (beam.Map(parse_line4))

                list_records | 'WriteTmpFile4' >> beam.io.WriteToText('gs://rockstardotb/tmp/list_records', file_name_suffix='.txt')

                tuple_records = list_records | 'CreateTupleRecords4' >> (beam.FlatMap(parse_records))

                tuple_records | 'WriteTmpFile5' >> beam.io.WriteToText('gs://rockstardotb/tmp/tuple_records', file_name_suffix='.txt')

                bigquery_records = tuple_records | 'CreateBigQueryRecord4' >> beam.Map(create_bigquery_record)

                bigquery_records | 'WriteTmpFile6' >> beam.io.WriteToText('gs://rockstardotb/tmp/bq_records', file_name_suffix='.txt')

                bigquery_records | 'WriteBigQuery4' >> beam.io.Write(
